...
NexT.utils = {
  registerImageLoadEvent: function () {
    const images = document.querySelectorAll(
      ".sidebar img, .post-block img, .vendors-list img"
    );
  ...
  registerToolButtons: function () {
    ...
    const scrollbar_buttons = buttons.querySelectorAll(
      "div:not(#toggle-theme)"
    );
  ...
  slidScrollBarAnime: function (targetId, easing = "linear", duration = 500) {
    ...
    window.anime({
      ...
      scrollTop:
        targetId == "" || !targetObj
          ? 0
          : targetObj.getBoundingClientRect().top + window.scrollY,
    });
  },
  ...
  fmtSiteInfo: function () {
    ...
    if (readTimes) {
      ...
      if (daysCount >= 1) {
        timesLabel =
          daysCount +
          NexT.CONFIG.i18n.ds_days +
          parseInt((times - daysCount * day) / hour) +
          NexT.CONFIG.i18n.ds_hours;
      } else if (hoursCount >= 1) {
        timesLabel =
          hoursCount +
          NexT.CONFIG.i18n.ds_hours +
          (times - hoursCount * hour) +
          NexT.CONFIG.i18n.ds_mins;
      } else {
    ...
    if (lastPushDate) {
      const pushDateVal = this.diffDate(
        lastPushDate.getAttribute("data-lastPushDate"),
        1
      );
  ...
  fmtBusuanzi: function () {
    setTimeout(function () {
      ...
      if (bszUV) {
        ...
        bszUV2.innerHTML =
          "<img src='https://img.shields.io/badge/uv-" +
          uv_val +
          "-2dde98'></img>";
      }
      // pv处理
      ...
      if (bszPV) {
        ...
        bszPV2.innerHTML =
          "<img src='https://img.shields.io/badge/pv-" +
          pv_val +
          "-1cc7d0'></img>";
      }
    }, 800);
  ...
  updateSidebarPosition: function () {
    if (
      ...
      NexT.CONFIG.scheme === "Pisces" ||
      NexT.CONFIG.scheme === "Gemini"
    )
      return;
    ...
    if (typeof display !== "boolean") {
      ...
      display =
        NexT.CONFIG.sidebar.display === "always" ||
        (NexT.CONFIG.sidebar.display === "post" && hasTOC);
    }
    if (display) {
